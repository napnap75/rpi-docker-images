...
func sendMessage(wac *whatsapp.Conn, group string, message string, title string, thumbnail []byte) error {
	...
	fmt.Fprintf(os.Stdout, "Message %d sent\n", msgId)
	return nil
}
...
func testConnexions(param parameters) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating connection to WhatsApp: %v\n", err)
	}
	err = login(wac, param.whatsappSessionFile)
	if err != nil {
		return fmt.Errorf("Error logging in WhatsApp: %v\n", err)
	}
	...
	if param.whatsappGroup == "" {
		fmt.Fprintf(os.Stdout, "No group provided, showing all available groups\n")
		for _, chatNode := range wac.Store.Chats {
			fmt.Fprintf(os.Stdout, "%s | %s\n", chatNode.Jid, chatNode.Name)
		}

		return fmt.Errorf("No group provided")
	} else {
		_, exists := wac.Store.Chats[param.whatsappGroup]
		if (!exists) {
			return fmt.Errorf("Unknown group %s", param.whatsappGroup)
		}
	}
	...
	if err != nil {
		return fmt.Errorf("Error connecting to MySQL: %v\n", err)
	}
	defer db.Close()
	results, err := db.Query("SELECT Version();")
	if err != nil {
		return fmt.Errorf("Error executing MySQL query: %v\n", err)
	}
	defer results.Close()
	...
	if err != nil {
		return fmt.Errorf("Could not find Piwigo thumbnail directory: %v\n", err)
	}
...
func runLoop(param parameters) error {
	...
	for results.Next() {
		...
		if (albumDate.Month() == time.Now().Month()) && (albumDate.Day() == time.Now().Day()) {
			...
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error send message to WhatsApp for album '%s': %v\n", albumName, err)
				continue
			}
		}
	}
...
func main() {
	...
	for {
		...
		fmt.Fprintf(os.Stderr, "Sleeping for: %d\n", d)
		time.Sleep(d)
